        ...
        if slug:
            if not is_valid_slug(slug):
                raise HTTPException(status_code=400, detail={
                    "error": "Invalid slug! Use only letters, numbers, hyphens, and underscores (3-30 chars)",
                    "api_dev": "@ISmartCoder",
                    "api_updates": "@abirxdhackz"
                })
            short_code = slug
...
async def check_clicks(url: str = None):
    try:
        ...
        last_clicked = doc["last_clicked"].strftime("%Y-%m-%d %H:%M:%S") if doc.get("last_clicked") else "Never"
